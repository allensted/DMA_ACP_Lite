...
//我虽然写了中断初始化函数，但是并没有写CDMA的中断通道函数，因为目前用不到
int CDMA_Setup_Intr_System(XScuGic * IntcInstancePtr,XAxiCdma * CDmaPtr, u16 TxIntrId, u16 RxIntrId)
{
	int Status;
	XScuGic_SetPriorityTriggerType(IntcInstancePtr, TxIntrId, 0xA0, 0x3);
...
int CDMA_Intr_Enable(XScuGic * IntcInstancePtr,XAxiCdma *DMAPtr)
{

	/* Disable all interrupts before setup */

	XAxiCdma_IntrDisable(DMAPtr, XAXICDMA_XR_IRQ_ALL_MASK);

	/* Enable all interrupts */

	//XAxiCdma_IntrEnable(DMAPtr, XAXICDMA_XR_IRQ_ALL_MASK);
...
int CDMA_Intr_Init(XAxiCdma *DMAPtr,u32 DeviceId)
{
	int Status;
	XAxiCdma_Config *Config=NULL;

	Config = XAxiCdma_LookupConfig(DeviceId);
	if (!Config) {
//		xil_printf("No config found for %d\r\n", DeviceId);
		return XST_FAILURE;
	}
	...
	/* Initialize DMA engine */
	Status = XAxiCdma_CfgInitialize(DMAPtr, Config,Config->BaseAddress);
